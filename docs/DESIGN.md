...
### LLM実行基盤: ai-hub-agent-proxy

LLM分析は直接のAPI呼び出しではなく、ローカルの [ai-hub-agent-proxy](https://github.com/nobu007/ai-hub_agent_proxy) 経由で実行する。

- プライマリ: Claude Code、フォールバック: OpenCode
- APIキー・モデル設定は proxy 側の `.env` が所有（knowledge-bucket側では持たない）
- `analyzer.py` の `call_agent()` が `subprocess` で proxy を起動: `node <proxy>/dist/cli.js --quiet --timeout 3600 -p <prompt>`
- タイムアウト: デフォルト **1時間**（巨大READMEの分析に時間がかかるため）。タイムアウト時は再試行しない
- proxyパスは環境変数 `KB_AGENT_PROXY` で指定、未設定時は `~/ai-hub_agent_proxy/dist/cli.js` を自動検出

```text
kb analyze --retry-failed -w 4    # analysis.confidence のない文書を再分析（4並列）
kb ingest --analyze -w 4          # ingest時に自動分析（4並列）
kb doctor                         # QC4で分析カバレッジ（confidence未設定文書）を警告
```

`--workers` (`-w`) は `analyzer.analyze_documents_parallel()` のThreadPool経由で複数のproxy subprocessを並列起動する。各分析は独立したsubprocess（I/Oバウンド）のため、workers=Nでバルク分析のwall-clockは約N分の1になる。
...
kb ingest                     # inbox内の未処理アイテムを処理（--analyze でLLM分析も同時実行、-w/--workers N でN並列）
...
```bash
kb graph build                # 概念グラフを構築（--full で差分ではなく全件再構築）
kb concepts suggest           # concept noteの昇格候補を提案・生成
kb analyze                    # 文書のLLM分析プロンプトを構築（--retry-failed で未分析文書を再分析、
                              #   -w/--workers N でN並列）
kb health                     # グラフ品質メトリクス表示（--json）
kb doctor                     # データ品質チェック（front-matter・必須フィールド・重複source_key・
                              #   分析カバレッジ・インデックス整合性）。1件でも失敗すると非0で終了
...
`kb analyze --retry-failed` と `kb ingest --analyze` はどちらも `-w/--workers N` を受け取り、
ai-hub-agent-proxy のsubprocessをN並列で起動してバルク分析のwall-clockを約N分の1に短縮する。
...
9. inbox内ファイル削除
10. sourcesテーブル登録
11. `--analyze` 指定時：各新規文書に対してai-hub-agent-proxy経由でLLM分析を実行し、concepts/summary/importanceをfront matterに書き込む。proxyが未検出ならスキップ（環境変数 `KB_AGENT_PROXY` でproxyパスを指定）。`-w/--workers N` でN並列
