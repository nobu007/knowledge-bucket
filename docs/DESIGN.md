...
```text
kb analyze --retry-failed    # analysis.confidence のない文書を再分析
kb ingest --analyze          # ingest時に自動分析
```
...
9. inbox内ファイル削除
10. sourcesテーブル登録
11. `--analyze` 指定時：各新規文書に対してLLM APIを呼び出し、concepts/summary/importanceをfront matterに書き込む。APIキーは環境変数 `KB_LLM_API_KEY` から取得
